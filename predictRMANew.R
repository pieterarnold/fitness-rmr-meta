# This function adapted from the original predict.rma in package metafor written by Wolfgang Viechtbauer <wvb@metafor-project.org>
predict.rmaNew <- function (object, newmods, intercept, tau2.levels, gamma2.levels,
    addx = FALSE, level, digits, transf, targs, ...)
{
    ...
    if (is.null(newmods)) {
        ...
        if ((!x$int.incl && x$p == 1L) || (x$int.incl && x$p ==
            ...
            ##     "") && !is.null(colnames(x$X)) && all(colnames(x$X) !=
            ##     "")) {
            ##     colnames.mod <- colnames(x$X)
            ##     if (x$int.incl)
            ##       colnames.mod <- colnames.mod[-1]
            ##     pos <- unlist(sapply(colnames(X.new), function(colname) grep(colname,
            ##                                                                  colnames.mod, fixed = TRUE)))
            ##     print(pos)
            ##     if (anyDuplicated(pos))
            ##       stop("Multiple matches for the same variable name.")
            ##     if (length(pos) != ifelse(x$int.incl, x$p - 1,
            ##       x$p))
            ##       stop("Could not find a matching name for all variables in the model.")
            ##     colnames(X.new) <- colnames.mod[pos]
            ##     pos <- unlist(sapply(colnames.mod, function(colname) grep(colname,
            ##       colnames(X.new), fixed = TRUE)))
    ...
    if (inherits(object, "rma.mv") && x$withH && x$gamma2s >
        1)
