# This function adapted from the original forest.default in package metafor written by Wolfgang Viechtbauer <wvb@metafor-project.org>
forestNew <- function (x, vi, sei, ci.lb, ci.ub, annotate = TRUE, showweights = FALSE,
    xlim, alim, clim, ylim, at, steps = 5, level = 95, refline = 0, refband = NULL,
    ...
    psize, col, lty, cex, cex.lab, cex.axis, annosym, ...)
{
    ...
    if (!is.null(refband)) {
        rect(refband[1], ylim[1] - 5, refband[2], ylim[2] - 2, col = "grey80", border = "grey80")
    }
    ...
    text(xlim[1], rows, slab, pos = 4, cex = cex*0.7, col = col,
        ...)
    if (!is.null(ilab)) {
    ...
    if (annotate) {
        ...
        text(x = xlim[2], rows, labels = annotext, pos = 2, cex = cex*0.7,
            col = col, ...)
    }
    for (i in seq_len(k)) {
        if (is.na(yi[i]))
